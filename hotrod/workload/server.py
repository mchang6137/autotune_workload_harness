...
def run_experiment():
    ...
    if request_type == 'index':
        cmd = 'ab -q -n {} -c {} -s 9999 -l http://{}:80/ > output.txt'.format(num_index, concurrency_index, hostname)
    elif request_type == 'dispatch':
        cmd = 'ab -q -n {} -c {} -s 9999 -l http://{}:80/api/dispatch?customer=123 > output.txt'.format(num_dispatch, concurrency_dispatch, hostname)
    elif request_type == 'mapper':
        cmd = 'ab -q -n {} -c {} -s 9999 -l http://{}:80/map/location > output.txt'.format(num_dispatch, concurrency_mapper, hostname)
